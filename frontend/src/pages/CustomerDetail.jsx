            ...
            <div className="flex gap-1 shrink-0">
              ...
              {confirming ? (
                <>
                  ...
                  >Sure?</button>
                  <button onClick={() => setConfirming(false)} className="text-xs border px-1.5 py-0.5 rounded text-gray-500">No</button>
                </>
              ) : (
                <button
...
function InlineAddForm({ onAdd, suggestions, priceMap, disabled }) {
  ...
  if (disabled) return (
    <div className="mt-2 pt-2 border-t border-gray-100 text-xs text-gray-400 italic">Creating page...</div>
  );
    ...
    <div className="mt-2 pt-2 border-t border-gray-100">
      ...
      {type && (
        <form onSubmit={submit} className="flex flex-wrap gap-1.5 items-center mt-1">
          {type === "bill" && (
            <>
              <input data-testid="inline-bill-amount" type="number" min="0" step="0.01" placeholder="Amount (optional)"
          ...
          {type === "item" && (
            <>
              <AutoInput data-testid="inline-item-name" value={itemName} onChange={setItemName}
              ...
              <input data-testid="inline-item-qty" type="number" min="0" step="0.01" placeholder="Qty"
                value={qty} onChange={e => setQty(e.target.value)} className="border rounded px-2 py-1 text-sm w-16" />
              <input data-testid="inline-item-price" type="number" min="0" step="0.01" placeholder="Price"
                value={price} onChange={e => setPrice(e.target.value)} className="border rounded px-2 py-1 text-sm w-24" />
          ...
          {(type === "money_received" || type === "money_given") && (
            <>
              <input data-testid="inline-money-amount" type="number" min="0" step="0.01" placeholder="Amount *"
                value={amount} onChange={e => setAmount(e.target.value)}
                className="border rounded px-2 py-1 text-sm w-32" autoFocus />
              <input data-testid="inline-money-note" placeholder="Note (optional)" value={note} onChange={e => setNote(e.target.value)}
          ...
          {type === "note" && (
            <>
              <input data-testid="inline-note-text" placeholder="Note text *" value={note} onChange={e => setNote(e.target.value)}
...
export default function CustomerDetail() {
  ...
  // Data served instantly from cache if available
  const customer = customerMap[cid] || null;
  const pages = pagesMap[cid] || [];
  const pagesTotal = pagesTotalMap[cid] || 0;
  ...
  // Reset auto-expand flag when navigating to a different customer
  useEffect(() => {
    initialExpandDone.current = false;
    setExpandedPages(new Set());
  }, [cid]);

  // On mount: serve from cache instantly + background refresh
  useEffect(() => {
    // Always background-refresh to keep data fresh
    refreshCustomer(cid);

    // Pages: serve from cache instantly, refresh in background
    api.get(`/customers/${cid}/pages`, { params: { settled: false, page: 1, limit: LIMIT } })
      .then(r => {
        setPagesMap(prev => ({ ...prev, [cid]: r.data.pages }));
        setPagesTotalMap(prev => ({ ...prev, [cid]: r.data.total }));
      });

    // Suggestions (fast, customer-specific)
    Promise.all([
  ...
  }, [tab, cid]); // eslint-disable-line react-hooks/exhaustive-deps

  // ---- Fetch entries for a page ----
  async function fetchPageEntries(pid) {
    if (pid.startsWith("tmp_")) return;
    setLoadingEntries(prev => new Set([...prev, pid]));
    try {
      const r = await api.get(`/pages/${pid}/entries`);
      setPageEntriesMap(prev => ({ ...prev, [pid]: r.data }));
    } finally {
  ...
  function toggleExpandPage(pid) {
    setExpandedPages(prev => {
      ...
      if (next.has(pid)) {
        ...
        next.add(pid);
        if (!pageEntriesMap[pid] && !pid.startsWith("tmp_")) fetchPageEntries(pid);
      }
      return next;
    });
  }
  ...
  async function addPage(e) {
    ...
    setPagesMap(prev => ({ ...prev, [cid]: [tempPage, ...(prev[cid] || [])] }));
    setExpandedPages(prev => new Set([...prev, tempId]));
    setPageEntriesMap(prev => ({ ...prev, [tempId]: [] }));
    setShowAddPage(false);
    setNewDate(today());
    try {
      const res = await api.post(`/customers/${cid}/pages`, { date });
      setPagesMap(prev => ({ ...prev, [cid]: (prev[cid] || []).map(p => p.id === tempId ? res.data : p) }));
      setExpandedPages(prev => {
        const next = new Set(prev);
        next.delete(tempId);
      ...
      setPagesMap(prev => ({ ...prev, [cid]: (prev[cid] || []).filter(p => p.id !== tempId) }));
      setExpandedPages(prev => { const n = new Set(prev); n.delete(tempId); return n; });
      setPageEntriesMap(prev => { const n = { ...prev }; delete n[tempId]; return n; });
    }
  }

  // ---- Optimistic delete page ----
  async function deletePage(pid) {
    const backup = [...(pagesMap[cid] || [])];
    setPagesMap(prev => ({ ...prev, [cid]: (prev[cid] || []).filter(p => p.id !== pid) }));
    ...
    try {
      ...
      setPagesMap(prev => ({ ...prev, [cid]: backup }));
    }
  }
  ...
  async function addEntry(pid, payload) {
    ...
    setPageEntriesMap(prev => ({ ...prev, [pid]: [...(prev[pid] || []), tempEntry] }));
    try {
  ...
  async function deleteEntry(pid, eid) {
    const backup = [...(pageEntriesMap[pid] || [])];
    setPageEntriesMap(prev => ({ ...prev, [pid]: (prev[pid] || []).filter(e => e.id !== eid) }));
    try {
      await api.delete(`/entries/${eid}`);
      refreshCustomer(cid);
    } catch {
      setPageEntriesMap(prev => ({ ...prev, [pid]: backup }));
    }
  }
  ...
  async function settleSelected() {
    ...
    setPagesMap(prev => ({ ...prev, [cid]: (prev[cid] || []).filter(p => !ids.includes(p.id)) }));
    setSelected(new Set());
    try {
      ...
      api.get(`/customers/${cid}/pages`, { params: { settled: false, page: 1, limit: LIMIT } })
        .then(r => setPagesMap(prev => ({ ...prev, [cid]: r.data.pages })));
    }
  }
  ...
  // Only show loading screen if truly not in cache AND not loaded yet
    ...
    <div className="container">
      ...
      {tab === "pages" && (
        <div>
          <div className="flex gap-2 mb-3 items-center">
            <button data-testid="add-page-btn" onClick={() => setShowAddPage(!showAddPage)}
              className="bg-blue-600 text-white px-3 py-1.5 rounded text-sm">+ Add Page</button>
            {selected.size > 0 && (
              <button data-testid="settle-selected-btn" onClick={settleSelected}
                className="bg-orange-500 text-white px-3 py-1.5 rounded text-sm">
                Settle {selected.size} page{selected.size > 1 ? "s" : ""}
              </button>
            )}
          </div>

          <input data-testid="page-search"
          ...
          {showAddPage && (
            <form onSubmit={addPage} className="bg-white border rounded p-3 mb-3 flex gap-2 items-center">
              ...
              <input data-testid="new-page-date" type="date" value={newDate}
                onChange={e => setNewDate(e.target.value)} className="border rounded px-2 py-1 text-sm" />
          ...
          <div className="space-y-2">
            {pages
              ...
              .map(p => {
                ...
                const entries = pageEntriesMap[p.id] || [];
                return (
                  <div key={p.id} data-testid={`page-card-${p.id}`} className={`bg-white border rounded ${p._temp ? "opacity-70" : ""}`}>
                    <div className="flex items-center gap-2 p-2.5">
                      <input type="checkbox" data-testid={`select-page-${p.id}`}
                        checked={selected.has(p.id)} onChange={() => toggleSelect(p.id)}
                        className="w-4 h-4 shrink-0" disabled={!!p._temp} />
                      <div className="flex-1 cursor-pointer flex items-center gap-2"
                      ...
                      {!p._temp && (
                        <button data-testid={`open-page-${p.id}`}
                          onClick={e => { e.stopPropagation(); nav(`/customer/${cid}/page/${p.id}`); }}
                          className="text-xs text-blue-600 border border-blue-200 px-2 py-0.5 rounded hover:bg-blue-50">Full</button>
                      )}
                      {!p._temp && (
                        pendingDeletePage === p.id ? (
                          <>
                            <button data-testid={`confirm-delete-page-${p.id}`} onClick={() => deletePage(p.id)}
                              className="text-xs border px-2 py-0.5 rounded bg-red-500 text-white">Sure?</button>
                            <button onClick={() => setPendingDeletePage(null)}
                              className="text-xs border px-2 py-0.5 rounded text-gray-500">No</button>
                          </>
                        ) : (
                          <button data-testid={`delete-page-${p.id}`} onClick={() => setPendingDeletePage(p.id)}
                            className="text-xs text-red-500 border px-2 py-0.5 rounded hover:bg-red-50">Del</button>
                        )
                      )}
                    </div>
                    ...
                    {isExpanded && (
                      <div className="border-t px-3 py-2">
                        ...
                        {!isLoadingEnt && entries.map(e => (
                          <InlineEntryRow key={e.id} entry={e}
                            onDelete={(eid) => deleteEntry(p.id, eid)}
                            onUpdate={(eid, payload) => updateEntry(p.id, eid, payload)}
                          />
                        ))}
                        <InlineAddForm onAdd={(payload) => addEntry(p.id, payload)}
                          suggestions={suggestions} priceMap={priceMap} disabled={!!p._temp} />
                      </div>
                    )}
                  </div>
                );
          ...
          {pages.length < pagesTotal && (
            <button data-testid="load-more-pages" onClick={loadMorePages}
              className="mt-3 w-full border rounded py-2 text-sm text-gray-500 hover:bg-gray-50">
              Load more ({pagesTotal - pages.length} remaining)
            </button>
          )}
        </div>
      )}
      ...
      {tab === "summary" && (
        <div data-testid="summary-section">
          <input data-testid="summary-search"
            className="border rounded px-3 py-1.5 text-sm w-full mb-3 bg-white"
            placeholder="Search in summary..." value={summarySearch} onChange={e => setSummarySearch(e.target.value)} />
      ...
      {tab === "pending-bills" && (
        <div data-testid="pending-bills-section">
          ...
          <input data-testid="pending-bills-search"
            className="border rounded px-3 py-1.5 text-sm w-full mb-3 bg-white"
            placeholder="Search by bill number or date..."
            value={pendingBillSearch} onChange={e => setPendingBillSearch(e.target.value)} />
              ...
              .map(b => (
                <div key={b.id} data-testid={`pending-bill-${b.id}`} className="bg-white border rounded p-2.5 flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex gap-1.5 ml-auto items-center">
                    <input data-testid={`fill-bill-input-${b.id}`}
                      type="number" min="0" step="0.01" placeholder="Amount"
                      value={fillAmounts[b.id] || ""} onChange={e => setFillAmounts(p => ({ ...p, [b.id]: e.target.value }))}
                      className="border rounded px-2 py-1 text-sm w-28" />
                    <button data-testid={`fill-bill-btn-${b.id}`} onClick={() => fillBillAmount(b.id)}
                      className="bg-blue-600 text-white px-3 py-1 rounded text-sm">Fill</button>
                  </div>
                </div>
              ))}
          </div>
      ...
      {tab === "pending-prices" && (
        <div data-testid="pending-prices-section">
          ...
          <input data-testid="pending-prices-search"
            className="border rounded px-3 py-1.5 text-sm w-full mb-3 bg-white"
            placeholder="Search by item name..."
            value={pendingPriceSearch} onChange={e => setPendingPriceSearch(e.target.value)} />
            ...
            {pendingPrices
              ...
              .map(it => (
                <div key={it.id} data-testid={`pending-price-${it.id}`} className="bg-white border rounded p-2.5 flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex gap-1.5 ml-auto items-center">
                    <input data-testid={`fill-price-input-${it.id}`}
                      type="number" min="0" step="0.01" placeholder="Price"
                      value={fillPrices[it.id] || ""} onChange={e => setFillPrices(p => ({ ...p, [it.id]: e.target.value }))}
                      className="border rounded px-2 py-1 text-sm w-28" />
                    <button data-testid={`fill-price-btn-${it.id}`} onClick={() => fillItemPrice(it.id)}
                      className="bg-blue-600 text-white px-3 py-1 rounded text-sm">Fill</button>
                  </div>
                </div>
              ))}
      ...
      {tab === "price-memory" && (
        <div data-testid="price-memory-section">
          ...
          <input data-testid="price-memory-search"
            className="border rounded px-3 py-1.5 text-sm w-full mb-3 bg-white"
            placeholder="Search item name..." value={priceMemSearch} onChange={e => setPriceMemSearch(e.target.value)} />
      ...
      {tab === "settled" && (
        <div data-testid="settled-section">
          ...
          <input data-testid="settled-search"
            className="border rounded px-3 py-1.5 text-sm w-full mb-3 bg-white"
            placeholder="Search by date..." value={settledSearch} onChange={e => setSettledSearch(e.target.value)} />
            ...
            {settledPages
              ...
              .map(p => (
                <div key={p.id} data-testid={`settled-page-${p.id}`} className="bg-white border rounded p-2.5 flex items-center gap-2">
                  ...
                  <button data-testid={`unsettle-page-${p.id}`} onClick={() => unsettlePage(p.id)}
                    className="text-xs border px-2 py-1 rounded text-blue-600 hover:bg-blue-50">Unsettle</button>
                </div>
              ))}
          </div>
        </div>
